function [pose_x,pose_y,pose_th,V_x,V_y,dyobs] = pursuit_con(x_init,x_init_fm,obs,goal_set,fm,detect_R)
load('smtraj.mat','x','y');
% load('1.mat','pose_xp2f','pose_yp2f');
% pose_x1 = pose_xp2f;
% pose_y1 = pose_yp2f;
% load('2.mat','pose_xp2f','pose_yp2f');
% pose_x2 = pose_xp2f;
% pose_y2 = pose_yp2f;
load('smooth.mat','nodepoly_zind','polyroute','nodepoly','range','nodez','noderoute','O_x','O_y','P_x','P_y');
lh_dis = 0.1;  % look-ahead distance
...
Kinematic = [0.3,pi,0.2,50/180*pi,0.01,1/180*pi];
% direction,distance,velocity,prediction time
...
while T  > time && lastIndex > target_ind
    ...
    collision = false;
    target_ind = calc_target_index(x,y,cx,cy,Lf);
    ai = PIDcontrol(t_speed, v,Kp);
    [delta, target_ind,~,~] = pure_pursuit_control(x,y,yaw,v,cx,cy,target_ind,0.1,lh_dis,L); 
    state = [x,y,yaw,v,v / L * tan(delta)];
...
%     obpose(k,4) = ob_poses(3,2);
    if(norm([dyobst(1)-x dyobst(2)-y]) < 0.3)
%         [u,traj{k-1}] = DynamicWindowApproach(state,Kinematic,[5,2.5],cost_param,dyobst,0.5);
%         state(1) = state(1) + u(1,1)*cos(state(3))*dt;
%         state(2) = state(2) + u(1,1)*sin(state(3))*dt;
%         state(3) = state(3) + u(2,1)*dt;
%         state(4) = u(1,1);
%         state(5) = u(2,1);
        [vx,vy] = DWA(state(1),state(2),dyobst,state(4)*cos(state(3)),state(4)*sin(state(3)));
        V_x(N,k+1) = vx;
        V_y(N,k+1) = vy;
...
%      V_y(N,k+1) = v_next(2);
%      if k < size(pose_x1,2)
%         if norm([pose_x(end,k)-pose_x1(end,k),pose_y(end,k)-pose_y1(end,k)]) < 0.5 
%             collision = true;
%         end
%      end
%      if k < size(pose_x2,2)
%          if norm([pose_x(end,k)-pose_x2(end,k),pose_y(end,k)-pose_y2(end,k)]) < 0.5 
%             collision = true;
%          end
%      end
     %% Followers
    for i = 1:fol_num        
        ...
        % repulsion = [0 0];
        repulsion = compute_repulsion([pose_x(i,k),pose_y(i,k)],ob_pose,0.0001);        
...
for v = v_min:v_res:v_max     
    for w = w_min:w_res:w_max
        [dis,hit] = cal_obs_dis(x,obs,R);    
        [x,traj] = generate_traj(x,v,w,cost_param(4));  
        stopdis = cal_breakdis(abs(v),model); 
        % Directional deviation score
        heading = 180 - abs(atan2(goal(2)-x(2),goal(1)-x(1))/pi*180-x(3)/pi*180); 
        % velocity = abs(v);                     
...
%% Update motion state
function [x, y, yaw, v, w] = update(x, y, yaw, v, a, delta, dt, L)
    x = x + v * cos(yaw) * dt;
    y = y + v * sin(yaw) * dt;
...
% control the acceleration by the current speed and the target speed
a = Kp * (target_v - current_v);                 
end
 
%% Pure Pursuit Algorithm
function [delta, ind,tx,ty] = pure_pursuit_control(x,y,yaw,v,cx,cy,ind,~,lh_dis,L)
% ind = calc_target_index(x,y,cx,cy,Lf);
% if the target point does not exceed the range, assign a specific point as the target
if ind < length(cx)  %
    tx = cx(ind);
    ty = cy(ind);
 % if it exceeds, the last point is assigned to the target
else               
    tx = cx(ind); 
    ty = cy(ind);
    ind = length(cx) - 1;
...
if v < 0
    alpha = pi - (atan2((ty-y),(tx-x))-yaw);
else
    alpha = atan2((ty-y),(tx-x))-yaw;
end
% update the look-ahead distance
Lf = 0.1 * v + lh_dis;  
delta = atan2(2*L * sin(alpha)/Lf,1);        
end
...
for i = 1:N
    dx(i) = x - cx(i);
    dy(i) = y - cy(i);
    Distance(i) = abs( sqrt(dx(i)^2 + dy(i)^2));
end
% find the index where the smallest Distance is located
[~, ind]= min(Distance);    

% ensure that the robot always moves towards the goal and avoids backsliding
LL = 0;
...
function [vx,vy] = DWA(pose_x,pose_y,ob_pose,V_x,V_y)
pre_px = pose_x;
pre_py = pose_y;
pre_p = [pose_x pose_y];
delta_t = 0.1;
